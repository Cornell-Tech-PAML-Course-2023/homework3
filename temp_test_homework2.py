from pages import A_Explore_Dataset, B_Preprocess_Data, B_Train_Model, C_Test_Model, D_Deploy_App
import pandas as pd
import numpy as np
...
def test_model_metrics():
    ...
    for idx, model in enumerate(models):
        student_results[regression_methods_options[idx]] = C_Test_Model.compute_eval_metrics(
...
def test_plot_learning_curve():
    ...
    _, student_plot_learning_curve_mul = C_Test_Model.plot_learning_curve(
        X_train, X_val, y_train, y_val, models[0], test_metrics, regression_methods_options[0])
    _, student_plot_learning_curve_pol = C_Test_Model.plot_learning_curve(
        X_train, X_val, y_train, y_val, models[1], test_metrics, regression_methods_options[1])
    _, student_plot_learning_curve_rid = C_Test_Model.plot_learning_curve(
        X_train, X_val, y_train, y_val, models[2], test_metrics, regression_methods_options[2])
    _, student_plot_learning_curve_las = C_Test_Model.plot_learning_curve(
